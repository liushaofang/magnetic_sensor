...
//	startTimer_TimerB(10, executeAtTimeUp_TimerB);
	...
	while (1) {
		...
		if (SGRI_IS_SET(FLAG_PACKET_RECEIVED)) {
			...
			cnt++;
			msg.frame[9] = cnt;
                        if(msg.frame[2] == OP_UPLOAD_RAW_MAGNETIC_DATA)
                        {
                            memcpy(uartMsg, &msg.highAddr, 4);
                            uartMsg[0] = msg.frame[0];
                            uartMsg[1] = msg.frame[1];
                            uartMsg[2] = msg.frame[3];
                            uartMsg[3] = msg.frame[4];
                            uartMsg[4] = OP_UPLOAD_RAW_MAGNETIC_DATA_TO_UART;
                            memcpy(&uartMsg[5], &msg.frame[5], 6);
                            uartMsg[11] = rssi_value_2_power(msg.rxMetrics[0]);
                        }
			DISABLE_INTERRUPTS_SGR();
			writeToUart_BSP(uartMsg, 12);
			ENABLE_INTERRUPTS_SGR();
                        
                        //twinkleLed_LED(LED1, 1, 5);
		}
