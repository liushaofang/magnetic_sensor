...
extern int16_t g_low_delta_threshold;
extern int16_t g_high_delta_threshold;
...
//        
//	g_mag_param.nHighThresholdVal = 70;//210;
//	g_mag_param.nLowThresholdVal = 5;//15;
...
//	while(1);
	...
	pmsFlags = 0;
        ...
        //2014.06.16:   liusf add for hmc5883l restart
        resume_hmc5883l();
        reset_background_magnetic();
        //2014.06.16:   liusf add end
	...
	while(1) {
		...
		if(PMS_IS_SET(FLAG_GEOMAGNETIC_DATA_PREPARED)) {
			...
			getValue(&g_is_car_parked, geomagnetic_raw_data, RAW_DATA_LEN);
                        //给协调器发送数据包
                        composeTransPacket(OP_UPLOAD_RAW_MAGNETIC_DATA, g_nwk_param.father_addr);
                        forcedTransPacket(&g_transmit_msg);
                        delayInMs_BSP(50);
                    #if 0
				...
				if(g_rf_communication_state != TRUE){
					...
					//changeChannelNum_MRFI(g_nwk_param.channel);
                        ...
                        else if(g_cur_state == 1 && g_is_car_parked == 1)
                        {
                                g_nMagnetResetCounter += 1;
                                if(g_nMagnetResetCounter >= 800)
                                {
                                        g_nMagnetResetCounter = 0;
                                        reset_background_magnetic();
                                        g_cur_state = 0;
...
static void composeTransPacket(uint8_t operation_id, addr_t targetAddr) {
	...
	case OP_END_ACK_CONFIG:{
        ...
        case OP_UPLOAD_RAW_MAGNETIC_DATA:
                g_transmit_msg.frame[0] = (uint8_t)(g_u16DeltaXYZData & 0xFF);  //可以上传参数0
                g_transmit_msg.frame[1] = (uint8_t)(g_u16DeltaXYZData >> 8 & 0xFF);     //可以上传参数1
                g_transmit_msg.frame[3] = g_n_inc_fluctuation_count;    //可以上传参数2
                g_transmit_msg.frame[4] = g_n_dec_fluctuation_count;    //可以上传参数3
                /*    增加原始地磁数据值   */
                g_transmit_msg.frame[5] = geomagnetic_raw_data[0];
                g_transmit_msg.frame[6] = geomagnetic_raw_data[1];
                g_transmit_msg.frame[7] = geomagnetic_raw_data[4];
                g_transmit_msg.frame[8] = geomagnetic_raw_data[5];
                g_transmit_msg.frame[9] = geomagnetic_raw_data[2];
                g_transmit_msg.frame[10] = geomagnetic_raw_data[3];
